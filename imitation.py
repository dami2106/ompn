...
flags.DEFINE_integer('il_save_freq', default=50, help='Save freq')
flags.DEFINE_bool('il_no_done', default=False, help='Whether or not to use done during IL')
flags.DEFINE_string('model_path', default='', help='Path of the model used if il_demo_from_model is True')
...
def main_loop(bot, dataloader, opt, training_folder, test_dataloader=None):
    ...
    if FLAGS.model_path != '':
        ...
        if FLAGS.cuda:
            bot = bot.cuda()
        print('Using demo from model: {}'.format(bot))
    else:
        ...
        # Ensure model is in eval mode for final save
        bot.eval()
        with open(os.path.join(training_folder, 'bot_final.pkl'), 'wb') as f:
            torch.save(bot, f)
    ...
    for env_name in dataloader.env_names:
        ...
        for ep_idx, traj in enumerate(all_trajs):
            ...
            action_strs = [str(idx) for idx in actions_cpu.squeeze()]
...
def run(training_folder):
    ...
    # Save the checkpoint - ensure consistent state with training
    bot.eval()
