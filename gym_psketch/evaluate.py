...
# def extract_hierarchy(bot, traj, tol_threshold=0.5):
#     # traj: a DictList of one demonstration (shape [1,T])
#     # 1) run the model
#     result = []  
#     mems = bot.init_memory(traj.env_id[:,0])
#     for t in range(traj.action.shape[1]):
#         out = bot.forward(obs=traj[:,t], env_ids=traj.env_id, mems=mems)
#         result.append({
#             'action': traj.action[0,t].item(),
#             'p': out.p.squeeze(0).cpu()
#         })
#         mems = out.mems

#     ps = torch.stack([r['p'] for r in result], dim=0)  # (T, nb_slots+1)
#     # 2) find boundaries
#     true_use_ids = get_use_ids(traj.action.view(-1).cpu().numpy(), env_name)
#     boundaries = get_boundaries(ps, bot.nb_slots, threshold=tol_threshold,
#                                 nb_boundaries=len(true_use_ids))
#     # 3) extract flat sub-tasks
#     flat_actions = torch.tensor([r['action'] for r in result])
#     subtasks = get_subtask_seq(flat_actions, subtask=ID2SKETCHS[env_id], use_ids=boundaries)

#     # 4) assign each time-step to its max-prob slot
#     assigned_slots = ps[:, :-1].argmax(dim=1).tolist()
#     # group into slot‐segments
#     segments = []
#     cur_id, cur_start = assigned_slots[0], 0
#     for i, s in enumerate(assigned_slots[1:], 1):
#         if s != cur_id:
#             segments.append((cur_id, (cur_start, i)))  # slot id and (start, end)
#             cur_id, cur_start = s, i
#     segments.append((cur_id, (cur_start, len(assigned_slots))))

#     return {
#         'subtasks': subtasks,        # list of action‐sequences
#         'boundaries': boundaries,    # list of cut indices
#         'slot_segments': segments,   # list of (slot_id, (start,end)) tuples
#     }
